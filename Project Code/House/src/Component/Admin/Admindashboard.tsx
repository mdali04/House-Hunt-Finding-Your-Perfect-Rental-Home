...
import { dashboards as fetchDashboards } from "../../Component/Actions/Admin";

const Admindashboard: React.FC = () => {
  const dispatch = useDispatch();

  useEffect(() => {
    dispatch(fetchDashboards() as any);
  }, [dispatch]);
  ...
  const chartOptions1: any = {
    ...
    series: [{
      ...
      data: data?.appartmentMonthwise?.map((item: any) => item.count) || []
    }],
    xaxis: {
      categories: data?.appartmentMonthwise?.map((item: any) => item._id) || []
    }
  };
  ...
  const chartOptions2: any = {
    ...
    series: [{
      name: 'Offer Data',
      data: data?.rentalmonthwise?.map((item: any) => item.count) || []
    }],
    xaxis: {
      categories: data?.rentalmonthwise?.map((item: any) => item._id) || []
    }
  };
  ...
  const chartOptions3: any = {
    ...
    series: [{
      name: 'Customers',
      data: data?.customerMonthwise?.map((item: any) => item.count) || []
    }],
    xaxis: {
      categories: data?.customerMonthwise?.map((item: any) => item._id) || []
    }
  };
