...
const ViewRents = () => {
  ...
  const gettingrentals = (username:string) =>{
    dispatch(ViewRental(username) as any);
  }
          ...
          <tbody>
            ...
            {users && users.map((user:any, index:any) => (
              <tr key={index} className="border-b hover:bg-gray-100">
                ...
                {
                  user.status === 'active' ? <td className="px-4 py-2 border border-gray-300">Approved</td> : 
                <td className="px-4 py-2 border border-gray-300">
                    <select name="status" id=""
                      onChange={(e) => dispatch(UpdateRental(e.target.value,user._id) as any)}
                    >
