...
import { dashboardss } from '../../Component/Actions/Rental';
...
const Dashboard = () => {
  ...
  useEffect(() => {
    dispatch(dashboardss(rentals._id) as any);
  }, [dispatch, rentals._id]);

  const dashboards = useSelector((state: any) => state.Rental.dashboard);
  ...
  const chartOptions2: any = {
    ...
    series: [{
      ...
      data: data.monthWiseBookings.map((item: any) => item.count),
    }],
    xaxis: {
      categories: data.monthWiseBookings.map((item: any) => item._id),
    },
  };
  ...
  const chartOptions3: any = {
    ...
    series: [{
      ...
      data: data.monthWiseRentals.map((item: any) => item.count),
    }],
    xaxis: {
      categories: data.monthWiseRentals.map((item: any) => item._id),
    },
  };
